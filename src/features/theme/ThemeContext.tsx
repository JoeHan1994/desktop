...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      // 优先从 SQLite 读取
      if (isTauri()) {
        try { raw = await getSetting(STORAGE_KEY); } catch { /* 忽略 */ }
      }

      // 回退到 localStorage
      if (!raw) {
        try { raw = localStorage.getItem(STORAGE_KEY); } catch { /* 忽略 */ }
      }

      if (raw) {
        try { setThemeState({ ...DEFAULT_THEME, ...JSON.parse(raw) }); } catch { /* 忽略 */ }
      }
    }
    load();
  }, []);
  ...
  useEffect(() => {
    ...
    try { localStorage.setItem(STORAGE_KEY, json); } catch { /* 忽略 */ }

    // 写 SQLite（Tauri 生产环境）
    if (isTauri()) {
      setSetting(STORAGE_KEY, json).catch(() => { /* 忽略 */ });
    }
  }, [theme]);

  const setTheme = useCallback(
    (patch: Partial<Theme>) => setThemeState((t) => ({ ...t, ...patch })),
    []
  );
