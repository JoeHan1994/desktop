...
function PageTitle({ stage, title, desc }: { stage?: string; title: string; desc?: string }) {
	return (
		<div className="flex shrink-0 items-center gap-3">
			{stage && <span className="glass glass-chip px-2.5 py-0.5 card-label">{stage}</span>}
			<h1 className="text-sm font-semibold tracking-tight card-title">{title}</h1>
			{desc && <span className="hidden text-[11px] text-white/30 xl:block">— {desc}</span>}
		</div>
	);
}
...
function ProviderModal({
	...
	function handleProvider(p: ProviderType) {
		setForm((prev) => ({
			...
			provider: p,
			apiBaseUrl:
				prev.apiBaseUrl === PROVIDER_META[prev.provider].defaultUrl ? PROVIDER_META[p].defaultUrl : prev.apiBaseUrl,
		}));
	}
			...
			>
				...
				>
					...
					<div className="flex items-center justify-between border-b border-white/[0.08] px-6 py-4">
						<div>
							<h2 className="text-sm font-semibold card-title">
								{isEditing ? '编辑 Model Provider' : '添加 Model Provider'}
							</h2>
							...
							<div className="flex gap-2">
								{(Object.keys(PROVIDER_META) as ProviderType[]).map((p) => (
									<button
										...
										style={
											form.provider === p
												? { color: PROVIDER_META[p].color, borderColor: PROVIDER_META[p].color + '60' }
												: {}
										}
									>
										<span
...
function ModelSelector({
	...
	const effectiveSelected = selected && providers.find((p) => p.id === selected.id) ? selected : (providers[0] ?? null);
		...
		<div className="glass app-card rounded-xl p-3.5 space-y-2.5">
			...
			>
				<div className="flex items-center gap-2 min-w-0">
					<span className="inline-block h-2 w-2 shrink-0 rounded-full" style={{ backgroundColor: currentMeta.color }} />
							...
							{providers.map((p) => {
								...
								return (
									<button
										...
										className={`glass app-card-surface app-card-control glass-control flex w-full items-center justify-between rounded-lg px-3 py-2 ${
											p.id === current.id ? 'text-white' : 'text-white/60 hover:text-white/80'
										}`}
									>
										<div className="flex items-center gap-2 min-w-0">
											<span
			...
			<div className="flex items-center justify-between text-[10px] text-white/30">
				<span>Model</span>
				<span className="tabular-nums truncate max-w-[120px]" style={{ color: currentMeta.color }}>
					{current.model || '—'}
				</span>
			</div>
		</div>
...
function TokenViz({
	...
	const inputTokens =
		latestStats?.promptTokens ?? messages.filter((m) => m.role === 'user').reduce((s, m) => s + m.tokens, 0);
	const outputTokens =
		latestStats?.completionTokens ?? messages.filter((m) => m.role === 'assistant').reduce((s, m) => s + m.tokens, 0);
	...
	const avgInputTps = latestStats?.inputTps
		...
		: (() => {
				...
				return msgs.length ? (msgs.reduce((s, m) => s + (m.prefillPerSec ?? 0), 0) / msgs.length).toFixed(0) : '—';
			})();
	const avgOutputTps = latestStats?.outputTps
		...
		: (() => {
				...
				return msgs.length ? (msgs.reduce((s, m) => s + (m.tokensPerSec ?? 0), 0) / msgs.length).toFixed(1) : '—';
			})();
	...
	const rows = [
		...
		{ label: 'Output', tokens: outputTokens, color: '#a78bfa', pct: (outputTokens / maxTok) * 100 },
		{
			...
			tokens: totalTokens,
			color: '#e2e8f0',
			pct: Math.min(100, (totalTokens / ctxLimit) * 100 * (ctxLimit / Math.max(totalTokens, 1))),
		},
	];
			...
			<div>
				<div className="mb-1 flex items-center justify-between">
					...
					<span
						...
						style={{ color: ctxPct > 80 ? '#f87171' : ctxPct > 60 ? '#fbbf24' : '#34d399' }}
					>
					...
					{error && (
						...
						>
							...
							<button
								...
								className="ml-auto shrink-0 text-rose-400/50 transition-colors hover:text-rose-400"
							>
								<svg
									viewBox="0 0 10 10"
		...
		>
			<div className="flex items-start justify-between gap-3">
				<div className="min-w-0">
					<div className="flex items-center gap-2">
						<span className="glass glass-chip shrink-0 px-2 py-0.5 text-[10px] font-semibold text-white/45">
							{source.id}
						</span>
			...
			<div className="mt-3 flex items-center gap-2">
				...
				<div className="flex shrink-0 gap-1">
					{source.tags.map((tag) => (
						<span key={tag} className="glass glass-chip px-1.5 py-0.5 text-[9px] text-white/35">
							{tag}
						</span>
					))}
				</div>
		...
		<div className="grid grid-cols-[repeat(16,minmax(0,1fr))] gap-0.5">
			{cells.map((value, index) => {
				...
				const backgroundColor =
					value >= 0 ? `rgb(96 165 250 / ${opacity.toFixed(2)})` : `rgb(248 113 113 / ${opacity.toFixed(2)})`;
	...
	return (
		<motion.button
			...
			style={
				active
					? {
							borderColor: 'rgb(var(--accent-rgb) / calc(var(--glass-border-alpha) * 1.8))',
							backgroundColor: 'rgb(var(--accent-rgb) / calc(var(--glass-alpha) * 0.7))',
						}
					: undefined
			}
		>
	...
	return (
		<motion.button
			...
			style={
				active
					? {
							borderColor: 'rgb(var(--accent-rgb) / calc(var(--glass-border-alpha) * 1.9))',
							backgroundColor: 'rgb(var(--accent-rgb) / calc(var(--glass-alpha) * 0.78))',
						}
					: undefined
			}
		>
			<div className="flex items-start justify-between gap-3">
				<div className="min-w-0">
					...
					<div className="mt-1 text-[10px] uppercase tracking-wide text-white/30">
						{file.format} · {formatKbFileSize(file.size)}
					</div>
				</div>
				<span
					className={`glass-chip shrink-0 border px-2 py-0.5 text-[10px] ${completed ? 'glass-status-success text-emerald-300/80' : 'glass-status-warning text-amber-300/80'}`}
				>
					{completed ? '已完成' : '解析中'}
				</span>
			</div>
			...
			<div className="mt-3 grid grid-cols-2 gap-2">
				...
				<div className="app-card-surface rounded-xl px-2.5 py-2">
					...
					<div className="mt-0.5 text-base font-bold tabular-nums text-white">
						{Math.round(totalTokens / 100) / 10}k
					</div>
				</div>
			</div>
...
export function KnowledgeBaseManagerView() {
	...
	useEffect(() => {
		...
		const timers = parsingFiles.map((file) =>
			window.setTimeout(
				() => {
					setFiles((previousFiles) =>
						previousFiles.map((item) => (item.id === file.id ? { ...item, status: 'completed' } : item)),
					);
				},
