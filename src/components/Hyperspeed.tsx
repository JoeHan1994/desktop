...
class App {
  ...
  loadAssets(): Promise<void> {
    ...
    return new Promise(resolve => {
      // 新版 postprocessing 的 SMAAEffect 会自动加载贴图，无需手动预载。
      const searchURL = (SMAAEffect as any).searchImageDataURL;
      const areaURL = (SMAAEffect as any).areaImageDataURL;
      if (!searchURL || !areaURL) {
        resolve();
        return;
      }
      ...
      searchImage.src = searchURL;
      areaImage.src = areaURL;
    });
  }

  init() {
    // React Strict Mode 下会双次挂载：若本实例已在清理中被 dispose，
    // 其异步 loadAssets().then(init) 不应再对已释放的 composer 初始化。
